							...
							<tr>
								...
								<td>
									<select name="FHTYPE" id="FHTYPE" data-placeholder="请选择数据库"  style="vertical-align:top;width: 87px;" onchange="selectType(this.value)">
										<option value="single" <c:if test="${ pd.FHTYPE=='single'}">selected</c:if>>单表</option>
										<option value="tree" <c:if test="${pd.FHTYPE=='tree'}">selected</c:if>>树形</option>
										<option value="fathertable" <c:if test="${pd.FHTYPE=='fathertable'}">selected</c:if>>主表</option>
										<option value="sontable" <c:if test="${pd.FHTYPE=='sontable'}">selected</c:if>>明细表</option>
									</select>
								</td>
							</tr>
						</table>
